...
### LOAD KD DATA
kd_wl_list=[".412.5",".442.5",".490",".510",".555",".560",".665",".670",".681.25"]
...
fig,axs = plt.subplots(9,2, gridspec_kw = {'wspace':.5, 'hspace':.5})
...
for i,WL in enumerate(WL_list):
    ...
    input_file= "/m100_work/OGS20_PRACE_P_1/plazzari/Forward_Adjoint/BOUSSOLE_DATA/Kd" + kd_wl_list[i] + "BOUSSOLEFit.txt"
    df_kd  = pd.read_csv(input_file, delimiter="\t",skiprows = 0, engine='python')
    yyyymmdd_kd = df_kd['yyyymmdd'].values
    HHMMSS_kd   = df_kd['HHMMSS'].values
    kd          = df_kd['Kd'].values
    date_list_kd=[]
    for tt,mydate in enumerate(yyyymmdd_kd):
        datestr= mydate + ' ' + HHMMSS_kd[tt]
        dateobj=datetime.strptime(datestr, "%Y-%m-%d %H-%M-%S")
        date_list_kd.append(dateobj)
    ...
    sunz     = df_filter_depth['sunz'].values

    index_row=i
### First Column "a"
    ax=axs[index_row, 0]
#   axT=ax.twinx()
    ax.scatter(date_list,(a+b)/np.cos(np.pi/180.*sunz),s=0.8,marker='*',color='k')
    ax.scatter(date_list_kd,kd,s=0.8,marker='*',color='g')
