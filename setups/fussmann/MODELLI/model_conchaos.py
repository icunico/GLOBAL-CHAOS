...
dxvalue=np.linspace(0.1, 1.2, 20) 
dcvalue=np.linspace(0.1, 1.2, 20)
...
for i in range(len(dxvalue)):
	for j in range(len(dcvalue)):
		...
		model.parameters['D/dC2'].value=dx
...
#		sol = scipy.integrate.solve_ivp(dy, [0., t_ex], model.state, t_eval=t_eval)
		sol = scipy.integrate.solve_ivp(dy, [0., t_ex], model.state, t_eval=t_eval, max_step=20000)
		...
		y = sol.y.T
		#t = t_eval
		specie_da_controllare= [1,3,4,5] #['N', 'P1', 'P2', 'P3', 'C1', 'C2', 'X1', 'Y1', 'Z1' 'D']
		flag_neg = False
		...
		#chaos
		taux = np.logspace(np.log(1), np.log10(5000), 5, dtype=int)
		#calcolo indici
	    ...
	    # Calcola gli indici per ogni valore di taux
		    ...
		    for it, tx in enumerate(taux):
		        ...
		        # Stampa gli indici calcolati per la specie
		    print(f"Specie {iv}: H (Shannon Entropy) = {H}")
		    print(f"Specie {iv}: C (Complexity) = {C}")
				
		    if not np.isnan(H[it]) and not np.isnan(C[it]):
                        flag_chaos = True  # Se entrambi gli indici sono validi, imposta la flag a True
