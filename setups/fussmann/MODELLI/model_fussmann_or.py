...
dx_values = np.linspace(100, 100, 1)
dc_values = np.linspace(0.1, 0.1, 1)
...
for dx in dx_values:
    for dc in dc_values:	
       # Aggiorna i parametri del modello
        tau = dx  # Assegna il valore di dx a dX1
        dC1 = dc  # Assegna il valore di dc a dC1 (se necessario)
        # Stampa i parametri in esecuzione
        print(f"Esecuzione con tau={dx}, dC1={dc}")
        # Condizioni iniziali

        initial_conditions = [0.5, 0.0, 0.0, 0.5, 0.0, 0.0, 0.0, 0.0, 2, 6]
        ...
        specie_da_controllare= [0,3] #['P1', 'P2', 'P3', 'C1', 'C2', 'X1', 'Y1', $
        flag_neg = False
        for iv in specie_da_controllare:
                if np.any(y[:, iv]) < 0.0: 
                    flag_neg = True
                    print("Flag Negativo:", flag_neg)
        y=y[-1500:]
      ...
      # Controllo del rapporto std/mean solo per le specie selezionate
        ...
        print("Mean (Media) per specie selezionate:", mean_y)
        print("Standard Deviation (Deviazione standard):", std_y)
        print("Ratio (std/mean, se sopra threshold):", ratio)
      # Controllo della stabilità
        ...
        # Controllo della stabilità (se il massimo del rapporto std/mean è minore o uguale alla soglia)  
        print("Flag Stabilità:", flag_ss)

        # Controllo dell'estinzione (se almeno una specie ha una popolazione inferiore alla soglia di estinzione)
        print("Flag Estinzione:", flag_extin)
