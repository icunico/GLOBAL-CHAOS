...
dx_values = np.linspace(0.40, 0.60, 10)
dc_values = np.linspace(0.40, 0.60, 10)
...
for dx in dx_values:
    for dc in dc_values:	
       # Aggiorna i parametri del modello
        ...
        # Condizioni iniziali
        initial_conditions = [0.5, 0.0, 0.0, 0.5, 0.0, 0.5, 0.0, 0.0]
...
#            if np.max(y[:, iv] < 0.001):
        ...
        #chaos
        taux = np.logspace(np.log(1), np.log10(5000), 500, dtype=int)
        #calcolo indici
        ...
        if flag_extin:
            ...
            ax.scatter(dc, dx, c='b', label='chaos' if flag_chaos else '')
        else:
