...
#define USE_EMBEDDING 0  // 0=output diretto, 1=embedding 3D
...
RecurrencePair* select_y_by_recurrence(Vec3 *series,int N,int *n_pairs){
    ...
    for(int t1=0;t1<N-T_STEPS;t1++){
        if(t1-last_t1_used<T_STEPS) continue;
        for(int t2=t1+T_STEPS;t2<N;t2++){
            double d=norm(series[t2],series[t1]);
            if(d>=soglia_min && d<=soglia_max){
                int max_len=N-(t1>t2?t1:t2);
...
void lyap_tau_continuous_multi(RecurrencePair *pairs,int n_pairs,double *Tpred,double *Tcrit){
    ...
    if(tau_count==0){ *Tpred=*Tcrit=NAN; return; }
...
int main(){
    ...
    for(int r=0;r<RHO_COUNT;r++){
        double rho = RHO_MIN + r*(RHO_MAX-RHO_MIN)/(RHO_COUNT-1);
        printf("\n--- rho = %.6f ---\n", rho);
        ...
        if(ZERO_DETERMINISTIC) tempo_pred_trad=INFINITY;
        else if(lyap_trad>0) tempo_pred_trad=(1.0/lyap_trad)*log(DELTAMAX/DELTA0);
        else tempo_pred_trad=INFINITY;
        tempo_pred_trad_list[r]=tempo_pred_trad;
        ...
        tempo_pred_tau_list[r]=Tpred_tau;
        tempo_critico_list[r]=Tcrit;

        // Stampa completa
        printf("Lyapunov tradizionale: %f\n", lyap_trad);
        printf("Tempo predicibilità tradizionale: %f\n", tempo_pred_trad);
        printf("Tempo predicibilità tau: %f\n", Tpred_tau);
    ...
    for(int r=0;r<RHO_COUNT;r++)
        fprintf(f,"%f %f %f %f\n", RHO_MIN + r*(RHO_MAX-RHO_MIN)/(RHO_COUNT-1),
                tempo_pred_trad_list[r],tempo_pred_tau_list[r],tempo_critico_list[r]);
    fclose(f);
