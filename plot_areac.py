...
try:
    ...
    # Separa i punti con tau = 0
    zero_data = valid_data[valid_data['tau'] == 0]
    non_zero_data = valid_data[valid_data['tau'] > 0]
    ...
    print(f"\nPunti con τ = 0: {len(zero_data)} ({len(zero_data)/len(valid_data)*100:.2f}%)")

except Exception as e:
    print(f"Errore lettura file: {e}")
    exit(1)

# Crea colormap personalizzata: blu -> viola -> giallo
...
# Plot punti non-zero con colormap
sc = plt.scatter(non_zero_data['lon'], non_zero_data['lat'],
                c=non_zero_data['tau'], cmap=cmap_custom,
                s=30, alpha=0.8, edgecolors='none',
                vmin=0, vmax=26, label=f'τ > 0 ({len(non_zero_data)} punti)')

# Plot punti zero in rosso
plt.scatter(zero_data['lon'], zero_data['lat'],
           c='red', s=30, alpha=0.8, edgecolors='none',
           label=f'τ = 0 ({len(zero_data)} punti)')

cbar = plt.colorbar(sc, label='Tempo di predicibilità τ', ticks=[0, 5, 10, 15, 20, 25, 26])
cbar.ax.set_yticklabels(['0', '5', '10', '15', '20', '25', '26'])
...
# 3. Mappa con proiezione geografica (con colori personalizzati)
try:
    ...
    # Plot punti non-zero con colormap
    sc = ax.scatter(non_zero_data['lon'], non_zero_data['lat'],
                   c=non_zero_data['tau'], cmap=cmap_custom,
                   s=30, alpha=0.8, transform=ccrs.PlateCarree(),
                   edgecolors='none', vmin=0, vmax=26,
                   label=f'τ > 0 ({len(non_zero_data)} punti)')
    
    # Plot punti zero in rosso
    ax.scatter(zero_data['lon'], zero_data['lat'],
              c='red', s=30, alpha=0.8, transform=ccrs.PlateCarree(),
              edgecolors='none', label=f'τ = 0 ({len(zero_data)} punti)')

    cbar = plt.colorbar(sc, ax=ax, orientation='horizontal',
                       pad=0.05, aspect=40, shrink=0.8,
                       ticks=[0, 5, 10, 15, 20, 25, 26])
    cbar.ax.set_xticklabels(['0', '5', '10', '15', '20', '25', '26'])
...
try:
    ...
    if nlat * nlon == len(valid_data):
        ...
        # Plot heatmap con colori personalizzati
        plt.figure(figsize=(12, 8))
        im = plt.imshow(tau_grid, extent=[lons_unique.min(), lons_unique.max(),
                                         lats_unique.min(), lats_unique.max()],
                       origin='lower', aspect='auto', cmap=cmap_custom,
                       vmin=0, vmax=26)
        cbar = plt.colorbar(im, label='Tempo di predicibilità τ',
                           ticks=[0, 5, 10, 15, 20, 25, 26])
        cbar.ax.set_yticklabels(['0', '5', '10', '15', '20', '25', '26'])
