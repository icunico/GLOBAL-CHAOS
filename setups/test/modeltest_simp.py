...
model = pyfabm.Model('fabm_Guido.yaml')
...
# Cicla su tutte le variabili e le plottiamo
for variable in model.state_variables:
    # Estrai l'indice della variabile nello stato
    var_index = model.state_variables.index(variable)
    var_data = y[:, var_index]
    
    # Plotta la variabile come una linea nel grafico
    plt.plot(t, var_data, label=variable.name)  # Usa 'label' per aggiungere la legenda
...
plt.legend()  # Aggiungi la legenda per identificare le variabili
plt.tight_layout()
