...
# Estrazione specie
specie_da_controllare_nomi = ['P1/DW1','P1/DW2','P1/DW3','C1/DWC1','C1/DWC2', 'X/DWX1'] #'X/DWX1'
...
#Comparison con Maximul Lyap Exponent


flag_chaos = False
lyap_threshold = 0.0  # se il massimo esponente è > 0, caos

for iv in specie_da_controllare:
    T = y[:, iv]
    # Calcola il massimo esponente di Lyapunov sul segnale T
    try:
        mle = nolds.lyap_r(T, emb_dim=6, lag=1, min_tsep=10)
    except Exception as e:
        print(f"Errore nel calcolo Lyapunov per specie {iv}: {e}")
        mle = -np.inf

    print(f"Max Lyapunov exponent specie {variable_names[iv]}: {mle}")

    if mle > lyap_threshold:
        flag_chaos = True
        break
