...
dxvalue=np.linspace(150, 150, 1) 
dcvalue=np.linspace(0.7, 0.7, 1)
...
for i in range(len(dxvalue)):
	for j in range(len(dcvalue)):
		...
		t_eval = np.linspace(0, t_ex, 200000)
		sol = scipy.integrate.solve_ivp(dy, [0., t_ex], model.state, t_eval=t_eval, max_step=50000)
		...
		y = sol.y.T
		#t = t_eval
		specie_da_controllare= [1,4,5] #['N', 'P1', 'P2', 'P3', 'C1', 'C2', 'X1', 'Y1', 'Z1' 'D']
		flag_neg = False
		...
		std_y = np.std(y_controllo, axis=0)
                ...
                #chaos
		taux = np.logspace(np.log(1), np.log10(len(t_eval)//2), 20, dtype=int)
                #calcolo indici
