...
dcvalue=np.linspace(0.4, 0.4, 1)

fileoutput = 'result.nc'
f = nc.Dataset(fileoutput, mode='w')
f.createDimension('dx', len(dxvalue))
f.createDimension('dc', len(dcvalue))

dx_var = f.createVariable('dx', np.float32, ('dx',))
dc_var = f.createVariable('dc', np.float32, ('dc',))

# Scrivi i valori di dx e dc nel file
dx_var[:] = dxvalue
dc_var[:] = dcvalue

f.createDimension('coord', 2)  # dimensione per dx e dc
combinations = f.createVariable('combinations', np.float32, ('dx', 'dc', 'coord'))
combinations.long_name = 'Combinations of dx and dc'
combinations.units = 'dx, dc'

flag_neg=False
neg = f.createVariable('flag_neg', np.int8, ('dx', 'dc'))
neg.units = '1=True, 0=False'
neg.long_name = 'flag_neg'
f.variables['flag_neg'][:] = flag_neg

flag_extin=False
extin = f.createVariable('flag_extin', np.int8, ('dx', 'dc'))
extin.units = '1=True, 0=False'
extin.long_name = 'flag_extin'
f.variables['flag_extin'][:] = flag_extin

flag_ss=False
ss = f.createVariable('flag_ss', np.int8, ('dx', 'dc'))   
ss.units = '1=True, 0=False'
ss.long_name = 'flag_ss'
f.variables['flag_ss'][:] = flag_ss

flag_chaos=False
chaos = f.createVariable('flag_chaos', np.int8, ('dx', 'dc'))
chaos.units = '1=True, 0=False'
chaos.long_name = 'flag_chaos'
f.variables['flag_chaos'][:] = flag_chaos
...
for i in range(len(dxvalue)):
	for j in range(len(dcvalue)):
		...
		t_eval = np.linspace(0, t_ex, 60000)
		...
		y = sol.y.T
		#t = t_eval
		specie_da_controllare_nomi= ['P1/DW1','C1/DWC1','P1/DW2','C1/DWC2','P1/DW3'] #['N', 'P1', 'P2', 'P3', 'C1', 'C2', 'X1', 'Y1', 'Z1' 'D']
		variable_names = [var.path for var in model.state_variables]
		# Trova gli indici nella matrice y corrispondenti ai nomi scelti
		specie_da_controllare = [i for i, name in enumerate(variable_names) if name in specie_da_controllare_nomi]
		print(specie_da_controllare)
		...
		std_y = np.std(y_controllo, axis=0)
                #chaos
		taux = np.logspace(np.log(1), np.log10(len(t_chaos)//2), 40, dtype=int)
                #calcolo indici
	...
	#		ax.text(dc, dx, f"{P1_value:.2f}", fontsize=10, color='red', ha='left', va='bottom')  
		...
		#Salvere i risultati
		        ...
		        # Memorizza i risultati nel file NetCDF per questa combinazione di dx e dc

		f.variables['flag_neg'][i, j] = 1 if flag_neg else 0
		f.variables['flag_extin'][i, j] = 1 if flag_extin else 0
		f.variables['flag_ss'][i, j] = 1 if flag_ss else 0
		f.variables['flag_chaos'][i, j] = 1 if flag_chaos else 0


        	# Memorizza la combinazione dx, dc nel file
		combinations[i, j, 0] = dx
		combinations[i, j, 1] = dc
...
#   var = f.createVariable(ncvar, np.float64, ('time', 'z'))
