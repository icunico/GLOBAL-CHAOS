...
bins = 160
x_edges = np.linspace(0.05, 2.0, bins + 1)
#y_edges = np.linspace(0.05, 2.0, bins + 1)

# Calcola histogram2d per ogni colore
conteggi = {}
for colore in colori_da_usare:
    mask = (colori == colore)
    H, _ = np.histogram(dc[mask], bins=x_edges)
    conteggi[colore] = H
