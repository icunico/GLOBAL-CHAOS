...
dxvalue=np.linspace(0.1, 0.45, 10) 
dcvalue=np.linspace(0.1, 1.2, 10)
...
for i in range(len(dxvalue)):
	for j in range(len(dcvalue)):
		...
		#ratio = np.where((mean_y >= threshold) & (mean_y != 0), std_y / mean_y, 0)
		ratio = np.where(mean_y >= threshold, std_y / mean_y, 0)
		# Controllo della stabilità
		flag_ss = np.max(ratio) <= 0.005
		...
		flag_extin=False
		for  iv in specie_da_controllare:
			if np.max(y[:, iv] < 0.0001): 
                                flag_extin=True

#		for  iv in specie_da_controllare: 
#			if np.all(y[:, iv] < 0.0001):
#				flag_extin=True
